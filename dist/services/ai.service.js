...
let AIService = class AIService {
    ...
    async run(input) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, UTC_TIMESTAMP(6))`, [
        ...
        if (input.taskType === 'chat_suggestion' && input.userId) {
         ...
         VALUES (?, ?, 'assistant', ?, TRUE, UTC_TIMESTAMP(6))`, [(0, utils_1.uuid)(), input.userId, parsed.candidate_text]);
        }
        return {
    ...
    async streamChatSuggestion(input) {
        ...
        const timeout = setTimeout(() => controller.abort(), Math.max(10000, aiConfig.timeout_ms ?? config_1.config.ai.timeoutMs));
        let content = '';
        let usage = { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 };
        let finishReason = null;
        ...
        try {
            const baseUrl = String(aiConfig.base_url ?? config_1.config.ai.baseUrl).replace(/\/+$/, '');
            ...
            if (!response.ok || !response.body) {
                ...
                throw errors_1.errors.aiProviderError('DeepSeek 流式调用失败：' + response.status + ' ' + detail.slice(0, 300));
            }
            ...
            if (error instanceof errors_1.AppError)
                throw error;
            throw errors_1.errors.aiUnavailable('DeepSeek 流式调用失败或超时，请稍后重试');
        }
        finally {
        ...
        if (!finalText)
            throw errors_1.errors.aiUnavailable('DeepSeek 没有返回有效回复');
        const safety = this.checkSafety(finalText);
        if (!safety.passed)
            throw errors_1.errors.aiUnavailable('AI 建议被安全检查阻断：' + safety.reason);
        if (input.userId) {
            await this.database.execute('INSERT INTO chat_messages (id, user_id, sender_role, content, ai_assisted, created_at) ' +
                "VALUES (?, ?, 'assistant', ?, TRUE, UTC_TIMESTAMP(6))", [(0, utils_1.uuid)(), input.userId, finalText]);
        }
        return {
            suggestion_id: (0, utils_1.uuid)(),
