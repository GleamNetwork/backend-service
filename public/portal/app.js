(() => {
  ...
  const state = {
    ...
    volunteer: { profile: null, cases: null, capacity: null, schedule: null, selectedCase: null, detail: null, detailRefreshTimer: null, showcaseAccepted: false, showcaseMessages: [
      { role: 'user', time: '上午 9:28', content: '我连续几晚只睡了四五个小时，但今天还是很兴奋，脑子停不下来。' },
      { role: 'volunteer', time: '上午 9:29', content: '谢谢你告诉我这些。听起来这几天和你平时的节奏不太一样。我们先不急着判断，想先轻轻确认：你现在能保证自己安全吗？' },
      { role: 'user', time: '上午 9:31', content: '我现在安全，没有想伤害自己或别人的念头。只是有点烦躁，又很想一下子把三个新项目都开起来。' },
      { role: 'volunteer', time: '上午 9:32', content: '好，知道你现在安全。我们可以先把那三个项目放在纸上，不急着今天全启动；如果你愿意，也可以按自己的计划联系熟悉的医生或家人。' }
    ] },
  ...
  function renderShell() {
    const volunteerWorkspace = state.role === 'volunteer';
    const connectionText = volunteerWorkspace ? '志愿陪伴工作区' : state.apiStatus === 'online' ? 'API 已连接' : state.apiStatus === 'error' ? 'API 连接异常' : '正在检查 API';
    const sidebar = `${renderRoleSwitcher()}${renderNav()}<div class="sidebar-note"><strong>安全与权限</strong>${state.role === 'user' ? '日记、设备趋势和人工求助分别授权；没有设备也能主动求助。' : '权限由后端角色、片区、个案归属和授权范围共同决定。页面隐藏按钮不能替代服务端鉴权。'}</div>`;
    const volunteerNavigation = volunteerWorkspace ? `<nav class="volunteer-topnav" aria-label="志愿者工作区导航"><button class="volunteer-topnav-button${state.route !== 'intake' ? ' is-active' : ''}" type="button" data-route="overview">当前陪伴</button><button class="volunteer-topnav-button${state.route === 'intake' ? ' is-active' : ''}" type="button" data-route="intake">接单中心${state.volunteer.showcaseAccepted ? '' : '<span>1</span>'}</button></nav>` : '';
    return `<div class="app-shell"><header class="topbar${volunteerWorkspace ? ' volunteer-topbar' : ''}"><button class="brand" type="button" data-action="go-home" aria-label="返回当前入口首页"><span class="brand-mark">频</span><span><strong>同频</strong><small>${volunteerWorkspace ? '志愿陪伴工作区' : `联机工作台 · ${roleLabel(state.role)}`}</small></span></button>${volunteerNavigation}<div class="topbar-middle"><span class="connection ${state.apiStatus === 'online' ? 'is-online' : state.apiStatus === 'error' ? 'is-error' : ''}"><i aria-hidden="true"></i>${connectionText}</span>${volunteerWorkspace ? '' : `<span class="api-address">${esc(state.apiBase)}</span>`}</div><div class="topbar-actions">${volunteerWorkspace ? '' : '<button class="button secondary small" type="button" data-action="open-api">API 设置</button>'}<button class="button quiet small" type="button" data-action="logout">退出登录</button></div></header><div class="workspace${volunteerWorkspace ? ' volunteer-layout' : ''}">${volunteerWorkspace ? '' : `<aside class="sidebar">${sidebar}</aside>`}<main class="main"><div class="content-wrap">${renderView()}</div></main></div></div>`;
  }
  ...
  function renderNav() {
    const items = state.role === 'user' ? [['home','首页','陪伴'],['chat','聊天','聊天'],['diary','日记','日记'],['device','设备摘要','设备'],['exercise','舒缓跟练','跟练'],['questionnaire','可选自评','自评'],['support','求助与资源','支持']] : state.role === 'volunteer' ? [['overview','当前陪伴','工作']] : state.role === 'ai_config_admin' ? [['ai','AI 配置','AI']] : [['overview','辖区总览','总览'],['queue','升级与交接','队列'],['people','人员与排班','人员'],['audit','审计与资源','审计']];
  ...
  function renderVolunteerView() {
    if (state.route === 'intake') return renderVolunteerIntake();
    const profile = state.volunteer.profile || {};
    const displayName = esc(profile.display_name || '志愿者');
    const messages = state.volunteer.showcaseMessages || [];
    return `<section class="page-head volunteer-page-head"><div><div class="eyebrow">当前陪伴</div><h1>陪她把这阵变化说清楚</h1><p>先听见她的感受，再把选择权交还给她。</p></div><div class="page-actions"><button class="button secondary" type="button" data-action="refresh-volunteer">刷新工作区</button></div></section><section class="volunteer-workspace"><aside class="companion-profile" aria-label="当前个案介绍"><div class="companion-profile-head"><div class="companion-avatar" aria-hidden="true">小</div><div><p class="eyebrow">当前最新个案</p><h2>来自上海的小精灵</h2><span class="status good">已确认安全，正在陪伴</span></div></div><p class="companion-intro">她住在上海，有一份自己确认过的支持计划。最近几天睡得比平时少，却不太困；她感觉脑子转得很快，也比平常更容易烦躁。</p><section class="companion-context"><h3>这次对话已经知道的事</h3><ul><li>她说自己目前能保证安全，也愿意继续文字陪伴。</li><li>她觉察到近几天的节奏和往常不同，想先慢下来确认感受。</li><li>要不要联系姐姐、朋友或固定医生，由她自己决定。</li></ul></section><dl class="companion-facts"><div><dt>此刻状态</dt><dd>精力很足，心里有些兴奋和烦躁</dd></div><div><dt>她希望怎样被陪伴</dt><dd>先把感受说清楚，不急着被解释或劝好</dd></div><div><dt>现在最需要</dt><dd>慢慢整理今天的安排，再决定要不要联系现实中的支持</dd></div></dl><div class="companion-note"><strong>${displayName}的小提醒</strong><span>不替她判断、不讨论诊断或用药。若她觉得不安全、明显失控或身体很不舒服，先陪她联系现实中的人和专业支持。</span></div></aside><section class="volunteer-chat card" aria-label="与来自上海的小精灵聊天"><div class="companion-chat-head"><div><p class="eyebrow">正在聊天</p><h2>和小精灵聊聊</h2><p>她在线，慢慢回复就好。</p></div><span class="status good">陪伴中</span></div><div class="message-list volunteer-message-list" aria-live="polite">${messages.map(volunteerShowcaseMessageHtml).join('')}</div><section class="conversation-guide" aria-label="陪伴提示"><div><p class="eyebrow">陪伴提示</p><h3>这次可以怎么继续</h3></div><ol><li><b>先听</b><span>接住她对节奏变化和烦躁的描述，不急着下结论。</span></li><li><b>再确认</b><span>问问这些变化怎样影响今天的安排，也尊重她跳过问题。</span></li><li><b>共同选择</b><span>可以只整理一个最小计划，也可以联系她愿意找的现实支持。</span></li></ol><p>不需要一次把所有事情谈完。停下来、换个话题或先安静一会儿，都可以。</p></section><form class="composer volunteer-composer" id="volunteer-showcase-form"><label class="sr-only" for="volunteer-showcase-message">发送陪伴消息</label><input id="volunteer-showcase-message" name="content" maxlength="500" placeholder="写下一句温和的回应" required><button class="button" type="submit">发送</button></form></section></section>`;
  }

  function volunteerShowcaseMessageHtml(item) {
    const mine = item.role === 'volunteer';
    return `<div class="message${mine ? ' me' : ''}"><small>${mine ? '我' : '小精灵'} · ${esc(item.time || '刚刚')}</small>${esc(item.content || '')}</div>`;
  }

  function renderVolunteerIntake() {
    return `<section class="page-head volunteer-page-head"><div><div class="eyebrow">接单中心</div><h1>等你接下这段陪伴</h1><p>先看看她愿意分享的近况，也确认自己现在适合陪她聊一会儿。</p></div></section><section class="intake-layout"><article class="intake-case"><div class="intake-case-head"><div class="companion-avatar" aria-hidden="true">小</div><div><p class="eyebrow">最新陪伴请求</p><h2>来自上海的小精灵</h2><span class="status warn">等待你的回应</span></div></div><p class="intake-intro">她主动发起了文字陪伴。最近几天睡得比平时少，却不太困；她觉得脑子很快、有些烦躁，想先找一个人慢慢说说。</p><div class="intake-summary"><section><p>她已经确认</p><strong>此刻能保证安全</strong><span>愿意在聊天里继续说说近况</span></section><section><p>她希望得到</p><strong>有人先听一听</strong><span>不急着解释，也不需要马上给答案</span></section><section><p>这次可以留意</p><strong>节奏和平时不同</strong><span>若她觉得不安全或明显失控，优先回到现实支持</span></section></div><div class="intake-share-note"><strong>她愿意分享给你的内容</strong><p>此刻的感受、最近生活节奏的变化，以及她希望被怎样陪伴。诊断、用药和原始设备数据不在本次陪伴中展示。</p></div></article><aside class="intake-prep"><p class="eyebrow">接单前，先确认</p><h2>你准备好陪她一会儿了吗</h2><ol><li><b>有一段相对安静的时间</b><span>不必承诺很久，但尽量不要在匆忙中开始。</span></li><li><b>愿意先听，再一起选下一步</b><span>不替她判断，也不把她的感受变成结论。</span></li><li><b>知道何时转向现实支持</b><span>出现不安全、明显失控或严重身体不适时，不由志愿者单独处理。</span></li></ol><div class="intake-accept"><strong>如果现在适合</strong><span>接下后，会进入这位小精灵的陪伴对话。</span><button class="button" type="button" data-action="accept-showcase-case">接下这次陪伴</button></div></aside></section>`;
  }
  ...
  async function handleSubmit(event) {
    ...
    if (form.id === 'volunteer-showcase-form') { const content = form.content.value.trim(); if (!content) return showToast('先写下一句想对她说的话。', true); state.volunteer.showcaseMessages.push({ role: 'volunteer', time: '刚刚', content }); render(); return; }
  ...
  async function handleAction(actionTarget) {
    ...
    if (action === 'accept-showcase-case') { state.volunteer.showcaseAccepted = true; state.route = 'overview'; showToast('你已接下这段陪伴，正在进入对话。'); return; }
